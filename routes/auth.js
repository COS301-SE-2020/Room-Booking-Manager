...
import {
    ...
    createSelfSignedCertificateIfNotExists,
} from "../helpers/certificateHelper";
...
export async function createSubscription(token) {
    ...
    subscriptionConfiguration.expirationDateTime = new Date(
        Date.now() + 3600000
    ).toISOString();
    ...
    if (subscriptionConfiguration.includeResourceData) {
        ...
        await createSelfSignedCertificateIfNotExists(
            certificateConfiguration.relativeCertPath,
            certificateConfiguration.relativeKeyPath,
            certificateConfiguration.password
        );
        subscriptionConfiguration.encryptionCertificate = getSerializedCertificate(
            certificateConfiguration.relativeCertPath
        );
        subscriptionConfiguration.encryptionCertificateId =
            certificateConfiguration.certificateId;
    }
...
authRouter.get("/callback", async (req, res, next) => {
    try {
        ...
        res.redirect("/home.html?subscriptionId=" + subscriptionData.id);
    } catch (error) {
        res.status(500);
        next(error);
    }
...
authRouter.get("/signout/:subscriptionId", (req, res) => {
    ...
    getSubscription(
        ...
        async (dbError, subscriptionData, next) => {
            try {
                const subscriptionService = new SubscriptionManagementService(
